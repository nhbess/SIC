...
def contact_tiles(obj: Polygon, N: int, scaler=1, color='black', linestyle='-', alpha=0.5):
    for i in range(N):
        for j in range(N):
            x = i*scaler
            y = j*scaler
            ...
            if obj.contains(Point(i+0.5, j+0.5)):
                ...
                shapely.plotting.plot_polygon(tile, 
                 add_points=False, 
                 facecolor=color, #'none',  # Set facecolor to 'none'
                 edgecolor='none',  # Specify the desired edgecolor
                 alpha=alpha,
                 zorder=-1, 
                 linewidth=1,
                 linestyle=linestyle,
...
def membrane_tiles(obj: Polygon, N: int, scaler=1, color='black', linestyle='-', alpha=0.5):
        ...
        for j in range(N):
            if grid[j, i] != 1:
                ...
                if up + down + left + right > 0:
                    ...
                    shapely.plotting.plot_polygon(tile, 
                     add_points=False, 
                     facecolor=color,  # Set facecolor to 'none'
                     edgecolor='none',  # Specify the desired edgecolor
                     alpha=alpha,
                     zorder=-1, 
                     linewidth=2,
...
def figure_environment():
    ...
    contact_tiles(obj=tetrom.polygon, N=N, scaler = 1, color = COLORS.CONTACT_TILE, linestyle='none')
    membrane_tiles(obj=target.polygon, N=N, scaler = 1, color = COLORS.MEMBRANE, linestyle='none', alpha=1)
    contact_tiles(obj=target.polygon, N=N, scaler = 1, color = COLORS.TARGET_TILE, linestyle='none', alpha=0.9)
    ...
    plt.plot([], [], 's', label="Membrane tiles", color=COLORS.MEMBRANE)
...
def figure_behavior_multistep(data_patha = None):
    #----------------- DATA -----------------
    filename = data_patha.split('Images\Data\\')[-1].split('.')[0]
    with open(data_patha, 'r') as file:
        data = json.load(file)
    ...
    for step, ax in zip(range(n_steps), axes):
        ...
        if 'No_Rotation' in filename:
            signal_A = signal_A*0
        ax.imshow(signal_A, cmap=cm.Oranges, alpha=1, zorder=-1, extent=[0, N, 0, M], origin='lower')
    ...
    cbar = fig.colorbar(cm.ScalarMappable(cmap=cm.Oranges), cax=cbar_ax)
    cbar.set_label(f'Excitation Signal $s$', size=25)
    #cbar.set_ticks([0, 0.5, 1])
    cbar.ax.tick_params(labelsize=25)
    
    plt.tight_layout()
    plt.savefig(f'Images/Paper/{filename}.png', dpi=600, bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    datafiles = ['Images\Data\Data_Behavior_Rotation.json',
                 'Images\Data\Data_Behavior_No_Rotation.json',
                 #'Images\Data\Square_Data_Behavior_Rotation.json',
                 #'Images\Data\Square_Data_Behavior_No_Rotation.json',
                 ]
    for datafile in datafiles:
        figure_behavior_multistep(data_patha=datafile)