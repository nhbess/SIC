...
class Board:
    ...
    def draw(self, window):
        ...
        all_signal_A = np.array([tile.S for tile in all_tiles])
        normalized_signals = all_signal_A
        colormap = plt.cm.get_cmap('inferno')

        # Convert normalized signal values to colors using the Viridis colormap
        color_list = [colormap(signal) for signal in normalized_signals]

        # Convert RGBA colors to RGB integers
        color_list = [(int(color[0]*255), int(color[1]*255), int(color[2]*255)) for color in color_list]

        # Update the colors of the tiles
        for tile, color in zip(all_tiles, color_list):
            tile.color = color
        ...
        for tile in self.tiles:
            ...
            if tile.is_target:
                ...
                width = self.TILE_SIZE//10
                pygame.draw.rect(window, tile.color, tile.rect, width=width)

            else:
                width = 0
                pygame.draw.rect(window, tile.color, tile.rect, width=width)

                pygame.draw.rect(window, (50, 50, 50), tile.rect, width=self.TILE_SIZE//10)
